...
public class MenuSite
{
    ...
    private static Map<Object, LinkedList<Item>> requesters = new HashMap<Object, LinkedList<Item>>();
    ...
    private static final LinkedList<Item> menuBarContents = new LinkedList<Item>();
    ...
    public static void addLine(Object requester, String toThisMenu, String name, ActionListener listener)
    {
        ...
        /*
         * 'element' 필드가 이곳에 있는 이유는 else 절의 assert에 걸렸을 경우 메뉴를 생성하지 않도록 하기 위햇서이다. 이러한
         * 고려를 하지 않는다면 if 절과 else 절에서 아이템을 생성할 수 있을 것이다.
         */
    ...
    public static void removeMyMenus(Object requester)
    {
        ...
        Collection<Item> allItems = (Collection<Item>) (requesters.remove(requester));
    ...
    public static void setEnable(Object requester, boolean enable)
    {
        ...
        Collection<Item> allItems = (Collection<Item>) (requesters.get(requester));
    ...
    private static JMenuItem getSubmenuByName(String name, MenuElement[] contents)
    {
        ...
        {
            /*
             * 시스템은 빈 서브 메뉴의 경우에는 내부 팝업 메뉴를 생성한다. 이러한 경우 팝업의 내용에서 'name'을 찾아 본다. PopupMenu와
             * JMenuItem이 같은 인터페이스를 구현했다면, 이 작업이 훨씬 쉬었겠지만 아쉽게도 그렇지 않다. 클래스 어댑터(adapter)를
             * 사용하여 이 둘이 공통의 인터페이스를 구현하고 있는 것처럼 보이게 할 수도 없다. JPopupWindows는 내가 아니라 스윙이 생성하기
             * 때문이다.
             */
    ...
    private static void setLableAndShortcut(JMenuItem item)
    {
        ...
        if (nameMap != null && (label = (String) (nameMap.get(name))) != null)
        {
            ...
            {
                item.setText(name);
                Logger.getLogger("com.holub.ui").warning("Bad" + "name-to-label maep entry:" + "\n\tiput=[" + name + "="
            ...
            } else
            {
                ...
                if (shortcut != null)
                {
                    ...
                    {
                        item.setAccelerator(KeyStroke.getKeyStroke(shortcut.toUpperCase().charAt(0),
                    ...
                    } else
                    {
                        ...
                        else
                        {
                            Logger.getLogger("com.holub.ui").warning(
                                    "Malformed shortcut parent spectification " + "in MenuSite map file: " + shortcut);
                        }
                    }
                }
            }
    ...
    /*
     * private static class Debug { public interface Visitor { public void visit(
     * JMenu e, int depth ); }
     *
     * private static int traversalDepth = 1;
     *
     * public static void visitPostorder( MenuElement me, Visitor v ) { // 만약 실제 타입이
     * (JMenu와 같은 JMenuItem 상속체가 아닌) // JMenuItem이라면 이는 리프 노드이며, // 자식을 갖고 있지 않다.
     *
     * if( me.getClass() != JMenuItem.class ) { MenuElement[] contents =
     * me.getSubElements(); for( int i = 0; i < contents.length; ++i ) { if(
     * contents[i].getClass() != JMenuItem.class ) { ++traversalDepth;
     * visitPostorder( contents[i], v ); if( !(contents[i] instanceof JPopupMenu) )
     * v.visit( (JMenu) contents[i], traversalDepth); --traversalDepth; } } } } }
     */
}
